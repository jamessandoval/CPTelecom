...
module.exports = function(passport) {
    ...
    passport.use('local-signup', new LocalStrategy({
        ...
        connection.pool.query("SELECT * FROM user WHERE email = ?", [email], function(err, rows) {
            if (err)
                return done(err);
            if (rows.length) {
                ...
                bcrypt.hash(password, saltRounds, function(err, hash) {
                    ...
                    connection.pool.query(insertQuery, [newUserQuery.email, newUserQuery.password], function(err, rows) {
