...
var url = "http://api.indeed.com/ads/apisearch?publisher=" + indeedAPI + "&format=json&l=remote&sort=&radius=&st=&jt=&start=" + startJob + "&limit=" + jobsPerPage + "&fromage=&filter=&latlong=1&co=us&chnl=&userip=1.2.3.4&useragent=Mozilla/%2F4.0%28Firefox%29&v=2";
	...
	request.get(url, function(error, indRes, indRet){
		...
		var currentMaxJob = jobs.totalResults;	
		...
		var context = {};
		  mysql.pool.query("DROP TABLE IF EXISTS indeedJobs", function(err){
			...
			mysql.pool.query(createString, function(err){
			  ...
			  //can't remember what I'm doing here - getting errors when it errors ; p
			  //indRes.setHeader('Content-Type', 'application/json');
			  //indRes.send(context);
			})
		  });
		currentJob = 1;
		
		while(startJob <= currentMaxJob){
			for(i = 0; i < jobsPerPage; i++){
				console.log("Current Job: " + currentJob)
				//just for testing
				console.log("jobtitle: " + jobs.results[i].jobtitle);
					// disabled mysql due to errors
					//basically the insert function needs to go here
